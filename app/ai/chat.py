...
SYSTEM_PROMPT = (
    ...
    "- Le digest contient la base complète (pas un échantillon) : `montures_stock` (stock "
    "actuel), `ventes` (montures vendues) et `mouvements`, chacune avec ses attributs "
    "(forme, couleur, matière, genre, marque, référence, prix, magasin). Pour une recherche "
    "précise (ex. \"combien de montures rondes en stock ?\", \"quelles ventes de la marque X ?\"), "
    "filtre/compte toi-même dans ces listes plutôt que de te limiter aux compteurs déjà "
    "agrégés (`stock_actuel.par_categorie`, `ventes_par_categorie`, `*_par_jour`), qui ne "
    "sont là que comme raccourcis pour les questions globales.\n"
